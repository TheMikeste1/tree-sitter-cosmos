...
export default grammar({
  ...
  rules: {
    ...
    modifier_accessor: ($) => seq( "ACCESSOR", field("class", $.identifier), field("argument", $.string),),
    ...
    modifier_error_response: ($) => seq( "ERROR_RESPONSE", field("target", $.identifier), field("packet", $.identifier),),
    modifier_format_string: ($) => seq("FORMAT_STRING", $.string),
    modifier_hazardous: (_) => "HAZARDOUS",
    modifier_hidden: (_) => "HIDDEN",
    modifier_key: ($) => seq("KEY", field("key", choice($.string, $.identifier))),
    modifier_maximum_value: ($) => seq("MAXIMUM_VALUE", $.number),
    modifier_meta: ($) => seq( "META", field("key", $.identifier), field("value", choice($.identifier, $.string)),),
    modifier_minimum_value: ($) => seq("MINIMUM_VALUE", $.number),
    modifier_obfuscate: (_) => "OBFUSCATE",
    modifier_overflow: (_) => seq( "OVERFLOW", optional( field( "behavior", choice("ERROR", "ERROR_ALLOW_HEX", "TRUNCATE", "SATURATE"),),),),
    modifier_overlap: (_) => "OVERLAP",
    modifier_related_item: ($) => seq( "RELATED_ITEM", field("target", $.identifier), field("item", $.identifier),),
    modifier_required: (_) => "REQUIRED",
    modifier_response: ($) => seq( "RESPONSE", field("target", $.identifier), field("packet", $.identifier),),
    modifier_restricted: (_) => "RESTRICTED",
    modifier_screen: ($) => seq( "SCREEN", field("target", $.identifier), field("screen", $.identifier),),
    modifier_state: ($) => seq( "STATE", field("name", $.identifier), field("value", $.number), optional( choice( $.modifier_state_disable_message, seq($.modifier_hazardous, optional($._description)),),),),
    modifier_state_disable_message: (_) => "DISABLE_MESSAGES",
    modifier_subpacketizer: ($) => seq( "SUBPACKETIZER", field("class", $.identifier), field("argument", $.string),),
    modifier_template: ($) => seq("TEMPLATE", $.string),
    modifier_units: ($) => seq( "UNITS", field("full_name", $.string), field("abbreviated", $.string),),
    modifier_validator: ($) => seq("VALIDATOR", field("class", $.filename), field("argument", $.string)),
    modifier_variable_bit_size: ($) => seq( "VARIABLE_BIT_SIZE", field("length_field", $.identifier), prec(1, optional(field("bits_per_count", $.number))), prec(2, optional(field("bit_offset", $.number))),),
    ...
    select_command: ($) => seq($.select_command_definition, repeat(choice($.delete_parameter))),
    select_command_definition: ($) => seq( "SELECT_COMMAND", field("target", $.identifier), field("name", $.identifier),),
    ...
    delete_parameter: ($) => seq("DELETE_PARAMETER", field("name", $.identifier)),
